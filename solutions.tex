...
\exercise{1.16} Show that $(1 - w)(1 - w^2) \cdots (1 - \w^{p-1}) = p$ by considering \[ t^{p} - 1 = (t - 1)(t - \w) \cdots (t - \w^{p - 1})\]

\begin{proof}
    We consider
\[
f(t) = \frac{t^{p} - 1}{(t - 1)} = (t - \w) \cdots (t - \w^{p - 1}) \]

The right-hand side has the value of $\lim_{t \to 1} f(t)$; the left-hand side has a discontinuity at $t = 1$ of the form $0 / 0$ so L'Hôpital's rule applies:

\[
(1 - \w) \cdots (1 - \w^{p - 1}) = \lim_{t \to 1}  \frac{t^{p} - 1}{(t - 1)} = \frac{pt^{p-1}}{1} \Bigg|_{t = 1} = p
 \]
\end{proof}

\exercise{1.17} Suppose that $\Z[\w]$ is a UFD and $\pi \divides x + y\omega$.  Show that $\pi$ does not divide any of the other factors on the left-hand side of \[ (x + y)(x + y\w)(x + y\w^2)\cdots(x + y\w^{p-1}) = z^{p} \ (1) \].

\begin{proof}
If $\pi \divides x + y\omega$ and $\pi \divides x + y\omega^{k}$ ($k \neq 1$), then there exist $\alpha, \beta$ so that $\alpha\pi = x + y\omega$ and $\beta\pi = x + y\omega^{k}$.

\begin{eqnarray*}
    \alpha\pi - x - y\omega &=& \beta\pi - x - y\omega^{k} \\
    \pi(\alpha - \beta) &=& y\omega(1 - \omega^{k-1}) \\
    \pi(\alpha - \beta) \w^{p-1} &=& y(1-\omega^{k-1}) \\
    \pi(\alpha - \beta) \w^{p-1} \prod_{\begin{subarray}{c} 0 \le j \le p - 1 \\ j \neq k-1\end{subarray}} (1-w^{j}) &=& y(1-\w)(1- w^2)\cdots (1-\w^{p-1}) = yp \\
\end{eqnarray*}
(This relies on $\w^{p} = 1$ and $\w^{-1} = \w^{p-1}$.)  Therefore, $\pi \divides yp$.  Since $\pi$ divides the left-hand-side of (1), $\pi$ must divide the right-hand-side ($z$) as well.

As we are considering case 1 of Fermat's Conjecture, $(x,y,z,p) = 1$ so there exist $m, n \in \Z$ so that $mz + nyp = 1$; since $\pi \divides yp, z$, $\pi(m\star_1 + n\star_2) = 1$; therefore $\pi$ is a unit.  This contradicts $\pi$ being an irreducible prime of $\Z[\w]$.
\end{proof}

\exercise{1.18} Use Exercise 1.17 to show that if $\Z[\w]$ is a UFD then $x + y\w = u\alpha^{p}$, $\alpha \in \Z[\w]$, $u \in \Z[\w]^{\times}$.

\begin{proof}
Let $x + y\w$ have the factorization $u \cdot \pi_0^{n_0} \cdots \pi_{k}^{n_k}$; by 1.18, if $\pi_i \divides x + y\w$, then $\pi_i$ does not divide any other factor on the LHS of (1) and so $\pi_i \divides z^{p}$; factor of $\pi_i^{n_i}$ occurring in the unique factorization of $z$ is a factor of $(\pi_i)^{pn_i}$ dividing $z^{p}$.  Therefore $x + y\w = u(\alpha^{p}$, where $\alpha = \prod \pi_i^{p n_i}$.
\end{proof}

\exercise{1.19} Dropping the assumption that $\Z[\w]$ is a UFD but using the fact that ideals factor uniquely (up to order) into prime ideals, show that the principal ideal $(x + y\w)$ has no prime ideal factor in common with any of the other principal ideals on the left side of the equation

\[ ( x + y)(x + y\w) \cdots (x + y\w^{p-1}) = (z)^{p} \]

\begin{proof}
Let $P$ be a prime ideal such that $P \supset (x + y\w)$ and $P \supset (x + y\w^{k})$, $k \neq 1$.  Then $x + y\w, x + y\w^{k} \in P$ and so $y\omega(1 - \w^{k-1}) \in P$.  Multipliying through by $\w^{p-1}$ and the other factors from equation (1), $yp \in P$; as $P$ is prime, either $y \in P$ or $p \in P$.  However, $z \in P$ (as $P \supset (z)^p$), contradicting $(x, y, z, p) = 1$.
\end{proof}

\exercise{1.20} Use Exercise 1.19 to show that $(x + y\w) = I^{p}$ for some ideal $I$.

\begin{proof}
The proof is exactly the same as the proof for Exercise 1.18, except that the unit factor doesn't occur in the prime decomposition.  Each ideal dividing $(x + y\w)$ does not share a factor with any other ideal dividing $(z)^{p}$, so $(x + y\w) = P_0^{pn_0} \cdots P_{k}^{pn_k}$; this is the required $I$.
\end{proof}

\exercise{1.21} Show that every member of $\Q[\w]$ is uniquely representable in the form \[ a_0 + a_1\w + \ldots + a_{p-2}\w^{p-2}, a_i \in \Q \] by showing $\w$ is a root of the polynomial \[ f(t) = t^{p-1} + t^{p-2} + \ldots + t + 1 \] and that $f(t)$ is irreducible over $\Q$.

\begin{proof}
Following the hint, $f(t + 1) = ((t + 1)^p  - 1)/t$ (follows from the closed form of the geometric series for $(t+1)^k$).  Using this identity and the Binomial Theorem, the $t^k$th term of the polynomial has coefficient $\binom{p}{k + 1}$ and so by Eisenstein's criterion, $f(t + 1)$ is irreducible.  $f(\w) = ((\w - 1 + 1)^{p} - 1)/(\w - 1) = (\w^p - 1)/(\w - 1) = 0$ and so it is a root of $f$.  Therefore $\w$ is a primitive element of $\Q[\w]$ and so $\{1, \ldots, \w^{p-2}\}$ forms a basis of $\Q[\w]$ over $\Q$, and any element $\alpha \in \Q[\w]$ can be written as $\alpha = a_0 + a_1\w + \ldots + a_{p-2}\w^{p-2}$.
\end{proof}

\exercise{1.22} Use Exercise 1.21 to show that if $\alpha \in \Z[\w]$ and $p \divides \alpha$, then (writing $\alpha = a_0 + a_1\w + \ldots + a_{p-2}\w^{p-2}, a_i \in \Z$), each $\modequiv{a_i}{0}{p}$.

\begin{proof}
If $p \divides \alpha$, write $\alpha = p\gamma$; write $\gamma = g_0 + g_1\w + \ldots + g_{p-2}\w^{p-2}$.  Termwise, each $a_i = pg_i$ and $a_i \in \Z[\w]$, so $g_i \in \Z[\w]$ and $p \divides a_i$.
\end{proof}

\exercise{1.23} Show that if $\modequiv{\beta}{\gamma}{p}$, then $\modequiv{\bar{\beta}}{\bar}{\gamma}{p}$, where the bar denotes complex conjugate.
\begin{proof}

If $\modequiv{\beta}{\gamma}{p}$, then $\beta - \gamma = \delta p$; therefore $\overline{\beta - \gamma} = \bar{\delta}\bar{p}$, so $\bar{\beta} - \bar{\gamma} = \bar{\delta}p$; therefore $\modequiv{\bar{\beta}}{\bar{\gamma}}{p}$.
\end{proof}

\exercise{1.24} Show that $\modequiv{(\beta + \gamma)^{p}}{\beta^p + \gamma^p}{p}$ and generalize this to arbitrary many terms by induction.

\begin{proof}
$(\beta + \gamma)^p = \beta^p + \gamma^{p} + \sum_{i=1}^{p - 1} \binom{p}{i} \beta^{i} \gamma^{p - i}$; each of the terms in the summation  are divisible by $p$ and so are 0 mod $p$, so $\modequiv{(\beta + \gamma)^{p}}{\beta^{p} + \gamma^{p}}{p}$.
\end{proof}

\exercise{1.25} Show that $\forall \alpha \in \Z[\w]$, $\alpha^{p}$ is congruent (mod $p$) to some $a \in \Z$.

\begin{proof}
Take $\alpha \in \Z[\w]$; then

\begin{equation}
\tag{Exercise 1.21}
\alpha = a_{p-2}\w^{p-2} + \ldots + a_0
\end{equation}

Raising both sides to the $p$-th power:

\[
\tag{Exercise 1.24}\modequiv{\alpha^{p}}{a_{p-2}^{p}\w^{p(p-2)} + \ldots + a_1^{p}\w^{p} + a_0^{p}}{p} \]

Since $\w^{p} = 1$, \[ \modequiv{\alpha^{p}}{a_{p-2}^{p} + \cdots + a_1^{p} + a_0^{p} \in \Z}{p} \]
This is the required result.
\end{proof}

For Exercises 1.26-1.28, assume $p \ge 5$, $\alpha \in \Z[\w]$, and let $u$ be a unit of $\Z[\w]$.  We assume the following: (proved in Exercise 2.12):
\begin{lemma}[Kummer's Lemma on Units]
If $u \in \Z[\w]^{\times}$, then $u / \bar{u}$ is a power of $\w$.
\end{lemma}

\exercise{1.26} Show that if $\modequiv{x + y\w}{u\alpha^{p}}{p}$, then $\modequiv{x + y\w}{(x + y\w^{-1})\w^{k}}{p}$ for some $k \in \Z$.

\begin{proof}
    By Exercise 1.25, $\modequiv{\alpha^p}{m}{p}$ for some $m \in \Z$, so $\modequiv{x + y\w}{um}{p}$.

    Applying Exercise 1.23 we have $\modequiv{x + y\w^{-1}}{\bar{u}m}{p}$; therefore $\modequiv{x + y\w}{(u/\bar{u})(x + y\w^{-1})}{p}$.  By the lemma on units $u / \bar{u} = \w^{k}$ for some $k$, so $\modequiv{x + y\w}{(x + y\w^{-1})\w^{k}}{p}$ for some $0 \le k < p$.
\end{proof}

\exercise{1.27} Use Exercise 1.22 to show that a contradiction results unless $\modequiv{k}{1}{p}$.

\begin{proof}
    \begin{eqnarray*}
        x + y \w &\equiv& (x+ y\w^{-1})\w^{k}\ (p)\ (\text{Exercise 1.26}) \\
        x + y\w - x\w^{k} - yw^{k-1} &\equiv& 0\ (p)
    \end{eqnarray*}
    This expression is divisible by $p$, so by Exercise 1.22 $p$ must divide all coefficients of this expression.
    If $k = 0$ or $k > 1$ then the constant term of the expression would be $x$, contradicting $x \not\mid p$.
\end{proof}

\exercise{1.28} Finally, show $\modequiv{x}{y}{p}$.

\begin{proof}
As $k = 1$, $\modequiv{(x - y) + (y - x)\w}{0}{p}$; by Exercise 1.22, $p \mid x - y$ and so $\modequiv{x}{y}{p}$.
\end{proof}

\exercise{1.29} Let $\w = e^{2\pi i / 23}$.  Verify that the product
\[ (1 + \w^2 + \w^4 + \w^6 + \w^{10} + \w^{11})(1 + \w + \w^{5} + \w^{6} + \w^{7} + \w^{9} + \w^{11}) \]
is divisible by 2 in $\Z[\w]$, although neither factor is.  It can be shown that 2 is an irreducible element in $\Z[\w]$; it follows that $\Z[\w]$ cannot be a UFD.

\begin{proof}
    We use Sage to avoid multiplying the terms out by hand:
\begin{verbatim}
...
For Exercises 1.30 - 1.32, let $R$ be an integral domain (commutative ring with 1 and no zero divisors).\\

\exercise{1.30} Show that two ideals in $R$ are isomorphic as $R$-modules if and only if they are in the same ideal class.

\begin{proof}
Let $I, J$ be ideals of $R$.

$\Rightarrow$: Let $\psi$ be an $R$-module isomorphism.  Take $i \in I$.  We show $\psi(i) I \sim i J$.

\[ \psi(i) I = \{\psi(i) i'\ |\ i' \in I\} = \{\psi(i i')\ |\ i' \in I \} = \{i\psi(i')\ |\ i' \in I \} = i J\]

(The last implication follows as $\psi$ is an isomorphism and so surjective.)

$\Leftarrow$: If $I \sim J$ then there exists $\alpha, \beta$ so that $\alpha I = \beta J$.  Therefore, for every $i \in I$, there is a $j \in J$ so that $\alpha i = \beta j$.  Define $\varphi(i) = j$.  We show this is an $R$-module isomorphism.

\begin{itemize}
    \item {\bf Injectivity:} Suppose $\varphi(i) = 0$; therefore $\alpha i = 0$.  As $R$ is an integral domain, either $\alpha = 0$ or $i = 0$.  If $\alpha = 0$, then $I, J$ are the zero ideals (so the only $i \in I = 0$).  So $\kernel(\varphi) = \{0\}$ and $\varphi$ is injective.
    \item {\bf Surjectivity:} Let $j \in J$; as $\alpha I = \beta J$ there is some $i \in I$ so that $\alpha i = \beta j$.  Therefore $\varphi(i) = j$ and so $\varphi$ is surjective.
    \item {\bf $R$-multiplicativity:} Suppose $\varphi(i) = j$; then for $r \in R$, $\alpha r i = \beta r j$ and so $\varphi(ri) = rj = r\varphi(i)$.
    \item {\bf Additivity:} For $1 \le n \le 2$, suppose $\varphi(i_{n}) = j_{n}$; then $\alpha (i_{n}) = \beta (j_{n})$, and so $\alpha(i_1 + i_2) = \alpha i_1 + \alpha i_2 = \beta j_1 + \beta j_2 = \beta(j_1 + j_2)$; therefore $\varphi(i_1 + i_2) = j_1 + j_2$.
\end{itemize}

\end{proof}

\exercise{1.31} Show that if $I$ is an ideal in $R$ and if $\alpha I$ is principal for some $\alpha \in R$, then $I$ is principal.  Conclude that the principal ideals form an ideal class.

\begin{proof}
Let $\alpha I = (\beta)$; then $\alpha I \sim (\beta)$ and by Exercise 1.30 there is an $R$-module isomorphism $\varphi : I \to (\beta)$.  Given $i \in I$, then $\varphi(i) = r\beta$ and $\varphi^{-1}(\varphi(i)) = \varphi^{-1}(r\beta)$.  Thus $i = r\varphi^{-1}(\beta)$ and $I$ is the principal ideal generated by $\varphi^{-1}(\beta)$.
\end{proof}

\exercise{1.32} Show that the ideal classes in $R$ form a group if and only if for each ideal $I$ there is an ideal $J$ such that $IJ$ is principal.

\begin{proof}
    In the ideal class group, the class containing the principal ideals (exists by Exercise 1.31) forms the identity.

    $\Rightarrow$: If the ideal classes form a group, then the inverse operation follows holds.

    $\Leftarrow$
\begin{itemize}
    \item {\bf Closed under operation:} Given ideals $I, J$, $IJ$ is another ideal.
    \item {\bf Identity exists:} The identity of the ideal class group is class containing the principal ideals.  If $I(\alpha) \sim I$ as $I(\alpha) = \alpha I$.
    \item {\bf Associativity:} $(IJ)K = I(JK)$ by associativity of element multiplication.
    \item {\bf Inverse exists:} This is an assumption of the $\Leftarrow$ hypothesis.
\end{itemize}
\end{proof}
...
\item[17.]  Let $K \subset L$; taking $\alpha \in L$, we have the tower of fields $K \subset K[\alpha] \subset L$; let $d = [K : K[\alpha]]$ and $n = [K : L]$.  Let $\{\beta_1, \beta_2, \ldots, \beta_{n/d}\}$ be a basis for $L$ over $K[\alpha]$; then $\bigcup_{\substack{0 \le m \le d - 1 \\ 1 \le k \le n / d}} \{\beta_k\alpha^m\}$ is a basis for $L$ over $K$.

As $\alpha \in L$, there is a polynomial $f$ so that $f(\alpha) = 0$; take $f = x^m + a_{m-1}x^{m-1} + \ldots + a_0$ (as $L$ is a field there is no issue with requiring $f$ to be monic).

Letting $A$ be the transformation matrix for multiplication by $\alpha$, the matrix for $A$ can be expressed in terms of this basis.  A sends $\beta_k \alpha_m$ to $\beta_k \alpha_{m+1}$, so for every $m < d - 1$, the $k + m$th column of the matrix.  When $m = d - 1$, the $m$ column is $\beta^{k}\alpha_{m}$; $\alpha_{m} = -a_{m-1}\alpha^{m-1} - \ldots - a_0$.  Therefore the matrix $A$ is made up of copies of the following matrix $B$ laid out diagonally.
\[
B = \left(
\begin{matrix}
    0 & 0 & \cdots & -a_0 \\
...
\right)
\]
$\trace{A} = \trace{B} \cdot (n/d) = -a_{m-1} (n/d$); as $-a_{m-1}$ is the first coefficient of the characteristic polynomial of $\alpha$, $-a_{m-1} = \text{tr}(\alpha)$; by Theorem 4', $\trace{A} = \text{Tr}^{L}_{K}(\alpha)$.

To compute $\det{A}$, we go column by column multiplying by the 1s in the diagonals of the $B$s; for column $m$ we take each $-a_0$.  Therefore $\det{A} = (-a_0)^{n/d}$, as $-a_0$ is the last coefficient of the characteristic polynomial of $\alpha$, $\det{A} = \text{n}(\alpha)^{n/d}$; by Theorem 4', $\det{A} = \text{N}^{L}_{K}(\alpha)$.
...
\right)
\]
    Let $f$ be a monic polynomial of degree $n$; e.g. $f = x^n + b_{n-1}x^{n-1} + \ldots + b_0$.  Apply the following series of column operations to the above matrix: add $b_{n-1}$ times the $n-1$th column to the $n$th column, add $b_{n-2}$ times the $n - 1$th column to the $n-2$th column, etc, the same determinant as the following.  None of these operations modifies the determinant of the resulting matrix; therefore
    \[
\det\left(\begin{matrix}
    1 & a_1 & \cdots & a_1^n \\
    1 & a_2^2 & \cdots & a_2^n \\
...
\right)
\]
    We now show the required result by induction.  Assuming the result for $n$, take
\[
    \det\left(\begin{matrix}
        1 & a_1 & \cdots & a_1^n \\
        1 & a_2^2 & \cdots & a_2^n \\
    ...
    \right)
\]
    Apply the above transform to $f = (x - a_1)(x - a_2) \cdots (x - a_n)$.  Then
\[
    \det\left(\begin{matrix}
        1 & a_1 & \cdots & a_1^n \\
        1 & a_2^2 & \cdots & a_2^n \\
    ...
    \det\left(\begin{matrix}
        ...
        1 & a_2^2 & \cdots & a_2^{n-1}  & 0\\
        \vdots & \vdots & & \ddots & \vdots \\
        1 & a_n & \cdots & a_{n}^{n - 1} & 0 \\
        1 & a_{n+1} & \cdots & a_{n+1}^{n-1} &  \prod_{1 \le r \le n} (a_{n-1} - a_r)  \\
    \end{matrix}
    \right)
    \]
Applying induction we have the required result.
...
\begin{gather*}
    ...
    \disc{\alpha} = -27m^2
\end{gather*}
Using Exercise 40, we see $-27m^2 = (d_1 d_2)^2 \disc{R}$ and $d_1^2 | d_2$, so writing $d_2 = d_1^2 j$, we have \[ -27m^2 = d_1^6 j^2 \disc{R} \]
